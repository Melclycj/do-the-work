...
| **See** | The file exists, its header carries the state machine, the four scopes, the three admission questions, the narrowing rule and the deletion discipline, and it has no entries. `dtw init` refuses to overwrite an existing one and names it in its report — a second run cannot clobber rulings. |
...
| | |
|---|---|
| **Do** | Two halves. **Tracked half:** put the hook script in the tree (`.githooks/pre-commit` is the convention both repositories use) and call the guards the caller wants from `<mount-path>/ResearchSystem/tooling/hooks/`. **Per-machine half:** `git config core.hooksPath .githooks`, once per checkout, by every person and every worktree. |
