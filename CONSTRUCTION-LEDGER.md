...
- **批 `PROMISE-PATH` 整体 CLOSED 2026-09-03**（两轮 `ENGINE` / `VOCAB`；行 2026-09-03 原样搬入
  [archive](CONSTRUCTION-LEDGER-archive.md)，链与裁决 1–12 在 plan `promise-path.plan.md`）。
  **下一队首＝候选隔离设计题**（是否开轮归用户，见下条），其后 dispatch-economy（携 `HD-69`）。
  **→ 队列于 2026-09-03 由用户改裁，见下一条 `CORE-MOUNT`。**
- **批 `CORE-MOUNT` CLOSED 2026-09-04（单轮；链、裁决与验收在 plan
  `document-harness/plans/core-mount.plan.md`，journal `document-harness/journal/core-mount-2026-09-03.md`）**：
  core-only 挂载做成调用者从挂载本身就能重做的一步（`document-harness/product-tier.txt` 随 tier travel +
  `ONBOARDING.md` 1b + 测试绑索引表）；顺带兑 `protected-set-says-five`。**轮中 FULL `B-1` 揭出契约
  §13.2 一句新假话（live-write-path 计数），用户遂裁两事**（载体各自）：① **契约签名挂起**
  （`CONTRACT-V4-SIGNATURE.md` SUSPENDED 段——§13 对草稿不适用，契约改动降为普通 `E2` 写；可逆,re-sign
  即复活）② **就地改契约族 `HD-63/64/67/68/70` 整族 retired 搬 archive**（授权改签名文本从此是签字记录里
  的一次事件、不再造 HD，呼应 2026-08-21「不造批准载体」）。`B-1` 遂降为普通 `E2` 写修毕，VERIFY
  `REVIEWED_NO_BLOCKER`（`E9` 一 FULL + 一修 + 一 VERIFY 花完）。**收批 `R5` 两条记待议**（归
  `EXECUTOR-LIFECYCLE` 期）：`F-2` 签名状态对调用者不可见（签字记录不 travel） · `O-1v` 要不要给
  `layer_path_check` 加「扫已提交树」模式使事后核验有意义。**下一队首＝`EXECUTOR-LIFECYCLE`**（候选隔离
  先做——executor 写入目标 + promote 形态；`HD-69` 同会话续跑与窄 subject 复读其后；九条 design rider
  打包收尾；其 plan 开批时写）。
