...
- **批 `CORE-MOUNT` OPEN 2026-09-03（单轮；链、裁决 1–4 与验收在 plan
  `document-harness/plans/core-mount.plan.md`，journal `document-harness/journal/core-mount-2026-09-03.md`）**：
  让 core-only 挂载成为调用者从挂载本身就能重做的一步——产品层路径清单随 tier travel
  （`document-harness/product-tier.txt`）、`ONBOARDING.md` 第 1 项写明 sparse-checkout 步骤、一个测试把
  清单绑到 `CONSTRUCTION-INDEX.md` 的产品层表；顺带兑 rider `protected-set-says-five`（独立 commit）。
  不裁分发形态，`HD-66` 照旧 live（本批只把其路 ① 做成可重复，不判 ① 是否够用）。
  **队列（用户 2026-09-03 裁，本行即承载）**：本批插队至候选隔离之前；候选隔离与 dispatch-economy
  （携 `HD-69`）**合为一批 `EXECUTOR-LIFECYCLE`**——隔离先做（executor 写入目标 + promote 形态），
  `HD-69` 同会话续跑与窄 subject 复读其后，九条 design rider 打包收尾；其 plan 开批时写。
