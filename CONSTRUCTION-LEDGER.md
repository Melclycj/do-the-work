...
- **批 `PROMISE-PATH` 立项，队首（用户 2026-09-01 三裁：批 `CORE-ONLY` 收口批准 · ISSUE_TRIAGE
  ...
  **2026-09-01 六问全裁**（1a 建路 · 2a 词表加值 · 3 确认 · 两轮 · 第七件并入〔`check_repair_decision`
  不验证，`HD-65` 边界之缺〕· 契约走族内第五裁），载 plan `promise-path.plan.md`；批开，
  轮 1＝`ENGINE`（件 3–7）未派。其后：候选隔离 · dispatch-economy（携 `HD-69`）。CLOSED 的
  `CORE-ONLY` 行与退休的 `R5` 条 2026-09-01 原样搬入 [archive](CONSTRUCTION-LEDGER-archive.md)。
